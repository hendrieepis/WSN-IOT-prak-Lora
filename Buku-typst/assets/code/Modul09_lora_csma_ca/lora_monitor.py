#!/usr/bin/env python3
"""
LoRa CSMA/CA Monitor Dashboard
==============================
Full-terminal live dashboard untuk monitoring komunikasi
LoRa Gateway <-> Node 1 & Node 2 (node mendengar kanal dulu, backoff acak, tanpa ACK)
...
# CSMA/CA (M08) belum punya ACK -- node tahu kanal sepi saat ia mengirim,
# tetapi tetap tidak tahu apakah paketnya sampai. Baris OK/FAIL/Retry di panel
# node diganti statistik akses kanal: sibuk, backoff, freeze, drop.
HAS_ACK          = False
# Ambang peringatan: node yang membuang paket sebanyak ini dianggap anomali.
DROP_WARN_STREAK = 3
...
class NodeState:
    ...
    retry_count:  int  = 0
    last_seq:     int  = -1
    busy_count:   int  = 0   # [CS] kanal SIBUK
    backoff_count:int  = 0   # [BACKOFF] berapa kali mundur acak
    freeze_count: int  = 0   # [FREEZE] pencacah backoff dibekukan
    drop_count:   int  = 0   # [DROP] paket dibuang, kanal tak kunjung bebas
    last_cw:      int  = 0   # contention window terakhir yang dipakai
    last_delay:   int  = 0   # tunda akses kanal paket terakhir (ms)
    avg_delay:    int  = 0   # rata-rata tunda akses kanal (ms), dari [STAT]
    raw_log: deque = field(default_factory=lambda: deque(maxlen=300))
...
# M08 CSMA/CA -- baris khas akses kanal di sisi node
RE_NODE_TUNDA  = re.compile(r'tunda=(\d+)\s*ms')
RE_NODE_BUSY   = re.compile(r'\[CS\] kanal SIBUK')
RE_NODE_BACKOFF= re.compile(r'\[BACKOFF\].*CW=(\d+)\s*\|\s*slot=(\d+)')
RE_NODE_FREEZE = re.compile(r'\[FREEZE\]')
RE_NODE_DROP   = re.compile(r'\[DROP\] SEQ=(\d+)')
RE_NODE_STAT   = re.compile(r'\[STAT\].*rata-rata tunda=(\d+)\s*ms')
...
def parse_node_line(line: str, st: NodeState, node_label: str, anomalies: deque):
    ...
    m = RE_NODE_TUNDA.search(line)
    if m:
        st.last_delay = int(m.group(1))

    m = RE_NODE_RETRY.search(line)
    if m:
        st.retry_count += 1

    if RE_NODE_BUSY.search(line):
        st.busy_count += 1

    m = RE_NODE_BACKOFF.search(line)
    if m:
        st.backoff_count += 1
        st.last_cw = int(m.group(1))

    if RE_NODE_FREEZE.search(line):
        st.freeze_count += 1

    m = RE_NODE_DROP.search(line)
    if m:
        st.drop_count += 1
        anomalies.append(Anomaly(ts, node_label, "DROP",
            f"SEQ={m.group(1)} dibuang -- kanal tetap sibuk sampai percobaan terakhir"))

    m = RE_NODE_STAT.search(line)
    if m:
        st.avg_delay = int(m.group(1))
...
def log_line_style(line: str) -> str:
    ...
    if '[DROP]'    in line: return "bold red"
    if '[FREEZE]'  in line: return "magenta"
    if '[CS]'      in line: return "yellow"
    if '[KALIBRASI]' in line: return "bold blue"
    if '[STAT]'    in line: return "dim cyan"
...
def build_node_panel(st: NodeState, label: str, color: str) -> Panel:
    ...
    if HAS_ACK:
        ...
        total = st.tx_count + st.drop_count
        t.append("  Akses kanal (CSMA/CA)\n", style="bold dim")
        t.append("  Kanal sibuk  : ", style="dim")
        t.append(f"{st.busy_count:>6}\n", style="yellow" if st.busy_count else "dim")
        t.append("  Backoff      : ", style="dim")
        t.append(f"{st.backoff_count:>6}\n", style="yellow" if st.backoff_count else "dim")
        t.append("  Freeze       : ", style="dim")
        t.append(f"{st.freeze_count:>6}\n", style="magenta" if st.freeze_count else "dim")
        t.append("  DROP         : ", style="dim")
        t.append(f"{st.drop_count:>6}\n", style="bold red" if st.drop_count else "dim")
        if total:
            kirim_pct = st.tx_count / total * 100
            t.append("  Terkirim     : ", style="dim")
            t.append(f"{kirim_pct:>6.2f}%\n", style=pct_style(st.tx_count, total))
        t.append("  CW terakhir  : ", style="dim"); t.append(f"{st.last_cw:>6}\n", style="white")
        t.append("  Tunda akhir  : ", style="dim"); t.append(f"{st.last_delay:>6} ms\n", style="white")
        t.append("  Tunda rata2  : ", style="dim"); t.append(f"{st.avg_delay:>6} ms\n\n", style="white")
        t.append("  (tanpa ACK -- lihat panel GATEWAY untuk\n", style="dim")
...
def build_layout(shared: SharedState, csv_path: str):
    with shared.lock:
        ...
        n1 = NodeState(
            ...
            busy_count=shared.n1.busy_count, backoff_count=shared.n1.backoff_count,
            freeze_count=shared.n1.freeze_count, drop_count=shared.n1.drop_count,
            last_cw=shared.n1.last_cw, last_delay=shared.n1.last_delay,
            avg_delay=shared.n1.avg_delay,
        ...
        n2 = NodeState(
            ...
            busy_count=shared.n2.busy_count, backoff_count=shared.n2.backoff_count,
            freeze_count=shared.n2.freeze_count, drop_count=shared.n2.drop_count,
            last_cw=shared.n2.last_cw, last_delay=shared.n2.last_delay,
            avg_delay=shared.n2.avg_delay,
...
BANNER = """\
╔══════════════════════════════════════════════════════════════════════╗
║          LoRa CSMA/CA   Monitor — Evaluasi Praktikum        ║
...
╚══════════════════════════════════════════════════════════════════════╝
  ...
  diterima per node, akses kanal (sibuk/backoff/freeze/drop) dan tunda
  akses, RSSI/SNR, deteksi anomali (GAP/DROP),
...
def parse_args(port_aktif):
    ...
    p = argparse.ArgumentParser(
        description="LoRa CSMA/CA Monitor Dashboard -- tool evaluasi praktikum LoRa.",
