...
Padanan Python dari Modul05_lora_master_slave/src/master/main.cpp: memanggil
...
Varian -withCRC: enableCrc()/disableCrc() menyalakan/mematikan bit RxPayloadCrcOn
(REG_MODEM_CONFIG_2 bit 2) -- padanan LoRa.enableCrc() sandeepmistry. Modul ini
memanggil enableCrc() saat init, sepasang dengan LoRa.enableCrc() di slave, jadi
paket berpayload rusak dibuang radio di kedua sisi, tidak lagi lolos ke aplikasi.
...
def enableCrc():
    _write_reg(REG_MODEM_CONFIG_2, _read_reg(REG_MODEM_CONFIG_2) | 0x04)


def disableCrc():
    _write_reg(REG_MODEM_CONFIG_2, _read_reg(REG_MODEM_CONFIG_2) & 0xFB)
