...
Alternatif untuk menjalankan manual:
    pio run -d week11_lorawan_chirpstack -e node1 -t upload -t monitor
    pio run -d week11_lorawan_chirpstack -e node2 -t upload -t monitor
...
gateway/single_chan_pkt_fwd.py, jadi tidak ada yang perlu di-upload untuknya.
    ...
    python3 week11_lorawan_chirpstack/upload_auto.py
    ...
    python3 week11_lorawan_chirpstack/upload_auto.py --only node2
        -> upload satu environment saja

    python3 week11_lorawan_chirpstack/upload_auto.py --node1 /dev/ttyACM0 --node2 /dev/ttyACM1
