...
- ☐ `master.py` berhasil memanggil kedua slave Uno bergiliran tanpa mengubah satu baris pun firmware slave — dibuktikan dengan `diff` terhadap `Modul05_lora_master_slave/src/slave/main.cpp`.
...
```bash
pio run -d Modul07_rpi_master_slave -e slave1 -t upload
...
**Verifikasi kesamaan kode** — bandingkan `src/slave/main.cpp` di modul ini dengan `Modul05_lora_master_slave/src/slave/main.cpp` memakai `diff`. Jawab: berapa banyak baris yang berbeda, apa isinya, dan mengapa tidak satu pun di antaranya menyentuh logika penyaringan `POLL:<SLAVE_ID>`?
