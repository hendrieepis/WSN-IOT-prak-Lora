...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   ...
   MODUL 09 — Random Backoff & Retry: Coba Lagi
...
| **09 (ini)** | **Random backoff + retry otomatis — `[FAIL]` bukan akhir cerita, gateway mengenali duplicate** |
| 10 | SYNC + slot waktu — tabrakan dihindari lewat penjadwalan terdesentralisasi, bukan diatasi lewat retry |
...
```bash
pio run -d week09_lora_aloha_retry -e gateway -t upload -t monitor
pio run -d week09_lora_aloha_retry -e node1   -t upload -t monitor
pio run -d week09_lora_aloha_retry -e node2   -t upload -t monitor
```
...
Persempit interval kirim kedua node (300–500 ms, seperti EXP-02 M08) supaya tabrakan lebih sering terjadi, lalu bandingkan hasil M08B dan M09 pada kondisi yang sama.

**Data capture**

| Parameter | M08B (tanpa retry) | M09 (dengan retry) |
|---|---|---|
...
> **CHECKPOINT** — Tingkat kegagalan permanen M09 harus jauh lebih rendah daripada tingkat `[FAIL]` M08B pada interval kirim yang sama. Bila keduanya mirip, periksa apakah `MAX_RETRIES` atau rentang backoff sudah benar ter-flash.
...
2. Dari tabel B, seberapa besar penurunan tingkat kegagalan dari M08B ke M09 pada interval yang sama? Jelaskan mengapa retry tidak pernah membuat kegagalan menjadi nol, hanya menurunkannya.
...
5. Apa perbedaan antara "gagal" pada M08B dan "gagal permanen" pada M09?
...
- **CH-3 — Statistik gap sesungguhnya.** Tambahkan penghitung total kirim (asli + retry) yang benar-benar mengudara per node di gateway (bukan hanya data baru), lalu hitung *airtime utilization* kanal untuk membandingkan ongkos radio M08B vs M09.
