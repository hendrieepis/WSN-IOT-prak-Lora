#!/usr/bin/env python3
"""Upload otomatis ke Modul 08A (Pure ALOHA) — port serial dideteksi sendiri.

Alternatif untuk menjalankan manual:
    pio run -d week08a_lora_pure_aloha -e gateway -t upload -t monitor
    pio run -d week08a_lora_pure_aloha -e node1   -t upload -t monitor
    pio run -d week08a_lora_pure_aloha -e node2   -t upload -t monitor
...
supaya tidak mungkin salah pasang.
    ...
    python3 week08a_lora_pure_aloha/upload_auto.py
        -> upload ketiga board, port otomatis, lalu tanya mau monitor atau tidak

    python3 week08a_lora_pure_aloha/upload_auto.py --monitor
    ...
    python3 week08a_lora_pure_aloha/upload_auto.py --only gateway
        -> upload satu environment saja

    python3 week08a_lora_pure_aloha/upload_auto.py --gateway /dev/ttyACM0 --node1 /dev/ttyACM1 --node2 /dev/ttyACM2
